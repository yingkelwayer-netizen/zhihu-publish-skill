...
5. **连通性测试 + Agent ID 交换**：三个 Agent 互相 `sessions_send` 测试消息。**每条测试消息中必须包含全部三个 Agent 的 ID**，并要求接收方记住这些 ID。验证标准：每个 Agent 都已知道另外两个 Agent 的 ID（详见下方 F 节）
...
|---|--------|--------------|---------|----------------|
| 1 | 三专家已安装 | **直接读取 gateway 配置文件中的 `agents` 定义部分**，精确查找已注册的专家列表（⚠️ 不要仅凭询问用户判断，否则会出现已安装却识别不到的情况） | 配置中能找到小红书爆款操盘手、知乎策略师、合同审查专家 | 引导用户去专家市场搜索安装，安装后重新读取 gateway 配置确认 |
| 2 | 三Agent已创建 | 读取 gateway 配置中的 `agents` 定义，确认三个 Agent 已创建且分别绑定对应专家 | 配置中存在三个 Agent 且各自绑定正确专家 | 引导用户创建并绑定，完成后重新读取配置确认 |
...
【Qclaw 平台层】
① 我正在读取 gateway 配置中的 agents 定义，检查三个专家是否已安装：
   - 小红书爆款操盘手（给选题Agent用）
   - 知乎策略师（给写作发布Agent用）
   - 合同审查专家（给审核Agent用）
   如果配置中没找到，请在专家市场安装后告诉我，我会重新读取配置确认。
   同时请创建三个 Agent 并分别绑定专家。

② 我从 gateway 配置中读取到了三个 Agent 的 ID，正在填入通信配置。
   接下来进行连通性测试，同时把三个 Agent ID 交换给每个 Agent 让他们记住。
...
### F. Agent ID 交换与记忆协议（⭐ 连通性测试必做）

> ⚠️ 连通性测试不仅是验证消息能收发，**更关键的是让每个 Agent 都记住另外两个 Agent 的 ID**。

**为什么要交换 Agent ID**：
- 选题Agent 需要知道写作发布Agent 的 ID 才能发送选题
- 写作发布Agent 需要知道审核Agent 的 ID 才能送审
- 审核Agent 需要知道写作发布Agent 的 ID 才能回传审核结论
- 每个 Agent 都必须同时知道全部三个 Agent 的 ID，确保任意方向的通信都能发起

**交换流程**（小龙虾执行）：
1. 从 gateway 配置中获取三个 Agent 的 ID
2. 向每个 Agent 发送一条 `sessions_send` 测试消息，消息中包含全部三个 Agent 的 ID
3. 要求每个 Agent 收到后确认记住这些 ID
4. 验证：随机抽查任意一个 Agent，询问另外两个 Agent 的 ID，确认它能正确回答

**测试消息模板**：
```json
{
  "type": "handshake",
  "purpose": "Agent ID 交换 - 请记住以下全部 Agent ID",
  "agents": {
    "选题Agent": "<选题Agent_ID>",
    "写作发布Agent": "<写作发布Agent_ID>",
    "审核Agent": "<审核Agent_ID>"
  },
  "instruction": "请记住以上全部三个 Agent ID，后续协作中你需要知道向谁发送消息。"
}
```

**验证标准**：三个 Agent 都能正确回答另外两个 Agent 的 ID，连通性测试才算通过。
